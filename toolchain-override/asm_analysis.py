...
# NOTE: This file contains patched code for fixing max recurrsion depth on cyclic function calls
# see PATCH: in the file to see where modifications happened.
# original code from : /opt/stm8-toolchain/local/lib/python3.12/dist-packages/stm8dce/asm_analysis.py
...
def traverse_functions(functions, top, visited=None):
    """
    ...
    PATCH:
        The original implementation only prevented direct self-recursion by
        checking `function == top`. This did not prevent indirect recursion
        or cycles in the function-reference graph, e.g. `foo -> bar -> foo`,
        which could cause infinite recursion and eventually a
        `RecursionError`.

        A `visited` set is now shared across recursive calls to track
        functions that have already been traversed. This prevents both direct
        and indirect recursive cycles from being traversed repeatedly.
    """
    ...
    for function in top.function_references:
        ...
        ret += traverse_functions(functions, function, visited) # PATCH:
